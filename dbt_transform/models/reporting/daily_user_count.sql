...
select date_trunc('day', event_time) as date
...
select *
from total_user_cte
left join miles_user_cte using (date)
left join searching_user_cte using (date)
left join engaging_user_cte using (date)